...
const css = `
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .del-btn{background:none;border:none;color:#475569;cursor:pointer;font-size:14px;padding:2px 6px;border-radius:4px;transition:all 0.1s;}
  .del-btn:hover{color:#f87171;background:rgba(248,113,113,0.08);}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      setUser(data.user)
      await loadWatchlist(data.user.id)
    })
  }, [])
  ...
  const loadWatchlist = async (userId: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const defaults = ['NVDA','AAPL','MSFT','TSLA','META']
  ...
  const removeFromWatchlist = async (sym: string) => {
    ...
    setWatchlist(prev => {
      ...
      if (selSym === sym && next.length > 0) selectStock(next[0])
      return next
    })
  }
  ...
  const addToWatchlist = async (r: any) => {
    ...
    <div style={s({background:'#0d0f12',minHeight:'100vh',color:'#e2e8f0',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',fontSize:'13px'})}>
      ...
      <div style={s({background:'#0d0f12',borderBottom:'1px solid #1e2530',padding:'0 20px',display:'flex',alignItems:'center',height:'52px'})}>
        ...
        <div style={s({marginLeft:'auto',display:'flex',alignItems:'center',gap:'10px'})}>
          ...
          <span style={s({fontSize:'11px',color:'#475569'})}>{user.user_metadata?.full_name || user.email}</span>
            ...
            <div style={s({background:'#13161a',border:'1px solid #1e2530',borderRadius:'12px',overflow:'hidden'})}>
              <div style={s({padding:'13px 16px',borderBottom:'1px solid #1e2530',display:'flex',alignItems:'center',justifyContent:'space-between'})}>
                <div>
                  ...
                  <div style={s({fontSize:'10px',color:'#475569',marginTop:'1px'})}>Gespeichert · Jede Aktie weltweit suchbar</div>
                </div>
              ...
              {addOpen && (
                <div style={s({background:'#191d22',borderBottom:'1px solid #1e2530',padding:'12px 14px'})}>
                  <input className="ainp" placeholder="Aktie suchen — Apple, Siemens, LVMH..." value={searchQ} onChange={e => handleSearch(e.target.value)} autoFocus />
                  ...
                  {searchQ.length >= 2 && !searching && searchResults.length === 0 && (
                    <div style={s({fontSize:'11px',color:'#475569'})}>Keine Ergebnisse.</div>
                  )}
                </div>
              )}
              {watchlist.map(sym => {
                ...
                return (
                  <div key={sym} className={`wl-item ${sym===selSym?'sel':''}`} onClick={() => selectStock(sym)}>
                    <span className="sym">{sym.replace('.DE','').replace('.PA','').replace('.L','')}</span>
                    <div style={s({minWidth:0,flex:1})}>
                      <div style={s({fontSize:'11px'})}>{st?.name || sym}</div>
                      <div style={s({fontSize:'9px',color:'#475569'})}>{st?.sector || ''}</div>
                    </div>
                    <div style={s({textAlign:'right' as const,marginRight:'8px'})}>
                      {lp ? (
                        <>
                      ...
                      <div style={s({display:'flex',alignItems:'flex-end',gap:'1.5px',height:'14px',marginTop:'3px',justifyContent:'flex-end'})} dangerouslySetInnerHTML={{__html:spark(st?.bars||[5,5,5,5,5,5,5],isNeg)}} />
                    </div>
                    <button className="del-btn" onClick={e=>{e.stopPropagation();removeFromWatchlist(sym)}} title="Entfernen">×</button>
                  </div>
                )
              })}
            </div>
            ...
            <div style={s({background:'#13161a',border:'1px solid #1e2530',borderRadius:'12px',overflow:'hidden'})}>
              <div style={s({position:'relative',padding:'20px 18px 16px',borderBottom:'1px solid #1e2530',overflow:'hidden'})}>
                <div style={s({position:'absolute',right:'-10px',top:'50%',transform:'translateY(-50%)',fontSize:'80px',fontWeight:900,opacity:0.04,color:'#e2e8f0',pointerEvents:'none',userSelect:'none' as const})}>
                  {selSym.replace('.DE','').replace('.PA','').replace('.L','')}
                </div>
                <div style={s({position:'relative',zIndex:1})}>
                  <div style={s({fontSize:'20px',fontWeight:700,color:'#6ee7b7'})}>{selSym.replace('.DE','').replace('.PA','').replace('.L','')}</div>
                  <div style={s({fontSize:'11px',color:'#94a3b8',marginBottom:'8px'})}>{selFund?.name || selStock?.name || selSym} · {selFund?.sector || selStock?.sector || ''}</div>
                  {loading ? (
              ...
              <div style={s({padding:'12px 14px'})}>
                <div style={s({fontSize:'9px',color:'#475569',letterSpacing:'1px',textTransform:'uppercase' as const,marginBottom:'8px'})}>Aktuelle News · {selSym.replace('.DE','').replace('.PA','')}</div>
                {loading ? (
                ...
                {stocks.map(st => {
                  ...
                  return (
                    <tr key={st.sym} className="cl" onClick={()=>{
                      if(!watchlist.includes(st.sym)){setWatchlist(prev=>[...prev,st.sym]);addToWatchlistDB(st.sym)}
                      setActiveTab('home');selectStock(st.sym)
                    }}>
...
function DCFCalculator() {
  ...
  const calc = () => {
    ...
    for(let y=1;y<=10;y++){rev*=(1+(y<=5?vals.g1:vals.g2)/100);pv+=rev*(vals.fcf/100)/Math.pow(1+vals.wacc/100,y)}
